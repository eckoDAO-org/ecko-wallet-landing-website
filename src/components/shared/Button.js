...
const StyledButton = styled(SUIButton)`
  ...
  font-size: ${({ fontSize }) => (fontSize ? fontSize + ' !important' : '46px !important')};
  color: ${({ color }) => (color ? color + ' !important' : '#000000 !important')};
...
const Button = ({ props, disabled, border, boxShadow, buttonStyle, background, color, fontSize, children, onClick, loading, hover }) => {
  return (
    <StyledButton
      {...props}
      disabled={disabled}
